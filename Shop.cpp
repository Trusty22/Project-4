...
int Shop_org::visitShop(int id) 
{
   ...
   print(id, "moves to service chair[" + int2string(barbers_.at(curBarber)->barber_id_) + "]. # waiting seats available = " + int2string(max_waiting_cust_ - waiting_chairs_.size()));
...
void Shop_org::leaveShop(int cusID, int barID) 
{
   ...
   print(cusID, "wait for barber[" + int2string(barID) + "]  to be done with the hair-cut");
